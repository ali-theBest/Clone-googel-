...
class _ReviewDetailsListState extends State<ReviewDetailsList> {
  ...
  children: [
    CircleAvatar(
      ...
      radius: 20,
      child: Text(
        widget.reviewList![index].name![0],
        style: TextStyle(color: Colors.white), // Ensure the text is visible
      ),
    ),
    SizedBox(width: 16), // Updated from `16.width` to `SizedBox(width: 16)`
    Expanded(
      child: Text(
        data.name!,
    ...
    PopupMenuButton<int>(
      onSelected: (int value) {
        if (value == 0) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Review flagged as inappropriate')),
          );
        } else if (value == 2) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Review flagged as spam')),
          );
        }
      },
      itemBuilder: (BuildContext context) {
        return [
          PopupMenuItem<int>(
            value: 0,
            child: Text('View edit history', style: primaryTextStyle()),
          ),
          PopupMenuItem<int>(
            value: 1,
            child: Text('Flag as inappropriate', style: primaryTextStyle()),
          ),
          PopupMenuItem<int>(
            value: 2,
            child: Text('Flag as spam', style: primaryTextStyle()),
          ),
        ];
      },
    ),
                    ...
                    children: [
                      Container(
                        ...
                        child: Text('Yes', style: primaryTextStyle(size: 14)),
                         ...
                         ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Thanks for the feedback')),
          );
                      }),
                      8.width,
                      Container(
